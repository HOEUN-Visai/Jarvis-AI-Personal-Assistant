...
class NLP:
    ...
    def is_question_with_modal(parts_of_speech):
        # Define grammar for detecting questions with modals
        grammar = 'QS: {<MD><PRP><VB>}'
        cp = nltk.RegexpParser(grammar)
        result = cp.parse(parts_of_speech)
        # Check if any subtree matches the question pattern
    ...
    def is_question_with_inversion(parts_of_speech):
        # Define grammar for detecting questions with verb inversion
        grammar = 'QS: {<VBP><PRP>}'
        cp = nltk.RegexpParser(grammar)
        result = cp.parse(parts_of_speech)
        # Check if any subtree matches the question pattern
    ...
    def _extract_verb(parts_of_speech):
        # Extract the first verb found in the parts of speech
    ...
    def _extract_modal(parts_of_speech):
        # Extract the first modal found in the parts of speech
        for part in parts_of_speech:
            if part[1] in ['MD']:
                return part[0]
        return ' '
    ...
    def _extract_noun(parts_of_speech):
        # Extract the first noun found in the parts of speech
        for part in parts_of_speech:
